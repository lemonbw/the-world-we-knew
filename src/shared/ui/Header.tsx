...
export default function Header() {
  ...
  return (
    <header
      className={`fixed z-10 flex h-[3.5rem] w-full items-center justify-between  px-2 duration-300 lg:duration-500 lg:static lg:mt-5 lg:block lg:h-[15vh] ${hiddenHeader ? '-translate-y-full' : 'translate-y-0'} lg:translate-y-0`}
    >
      <div className="top-0 left-2 z-10 flex h-[2.5rem] items-center justify-center lg:mt-4">
        <NavLink href="/" title="The World We Knew" as="h1" className='text-[1.2rem] font-bold lg:text-[2rem]' spanClassName='h-[2.5px]' />
      </div>
      <Hamburger hiddenHeader={hiddenHeader} hiddenNavigation={hiddenNavigation} setHiddenNavigation={setHiddenNavigation} />
      <nav className={`z-5 absolute lg:static flex-col lg:flex lg:flex-row lg:translate-x-0 justify-between ${hiddenNavigation ? "translate-x-full" : "translate-x-0"} duration-300 w-full mt-3 mb-4 -ml-2 pl-2 lg:ml-0 lg:pl-0 text-[1.3rem] font-medium ${hiddenHeader ? "translate-x-full" : "translate-x-0"}`}>
