          ...
          {sortedChapters.map((c) => (
            <tr
              ...
              <td className="relative px-4 pb-0 z-10 ">
                <Link href={c.href} className="block w-full h-full z-10 transition-colors duration-1000"
                  style={{ color: isPressed || hoveredIndex === c.index ? "black" : "white", }}>{c.volume}</Link>
                <span
              ...
              <td className="px-4 py-2 relative z-10">
                <Link href={c.href} className="block w-full h-full transition-colors duration-1000"
                  style={{ color: isPressed || hoveredIndex === c.index ? "black" : "white" }}>{c.chapter}</Link>
              </td>
              <td className="px-4 py-2 relative z-10">
                <Link href={c.href} className="block w-full h-full transition-colors duration-1000"
                  style={{ color: isPressed || hoveredIndex === c.index ? "black" : "white" }}>{c.title}</Link>
              </td>
              <td className="px-4 py-2 relative z-10">
                <Link href={c.href} className="block w-full h-full transition-colors duration-1000"
                  style={{ color: isPressed || hoveredIndex === c.index ? "black" : "white" }}>{c.symbols}</Link>
              </td>
              <td className="px-4 py-2 relative z-10">
                <Link href={c.href} className="block w-full h-full transition-colors duration-1000"
                  style={{ color: isPressed || hoveredIndex === c.index ? "black" : "white" }}>{c.date}</Link>
              </td>
            </tr>
          ))}
        </tbody>
